...
component RegDis is
    Port ( clock    : in  std_logic;                        -- Señal del clock (reducido).
           ...
           demux_address     : in  std_logic;                        -- Señal de carga.
...
with ram_address select
    ram_dataout <= muxin_s when "000000000100",
                   muxin_m when "000000000101",
                   muxin_u when "000000000110",
                   "0000000000" & d_btn when "000000000011",
...
with write_ram select
    demux_dis <= write_ram when '0',
                 '0' when others;

with write_ram select
    demux_led <= write_ram when '1',
                 '0' when others;
...
inst_RegDis: RegDis port map(
       ...
       demux_signal     => demux_dis,                        -- Señal de carga.
 ...
 inst_RegLED: RegLED port map(
       clock             => clk,                       -- Señal del clock (reducido).
       clear             => clear,                        -- Señal de reset.
       demux_signal     => demux_led,                        -- Señal de carga.
       up                => '0',                        -- Señal de subida.
       down              => '0',                        -- Señal de bajada.
       ram_datain        => ram_datain(15 downto 0),   -- Señales de entrada de datos.
       led               => ledout  -- Señales de salida de datos.
 );
